...
public class Usuario {
    ...
    public void alugarLivro(Livro livro, LivroDAO livroDAO) {
        int limite = tipo.equalsIgnoreCase("aluno") ? 3 : 5; // Definir limite baseado no tipo do usuário
        ...
        if (livro.isDisponivel()) {
            try {
                // Atualizar disponibilidade no banco de dados
                livroDAO.atualizarDisponibilidade(livro.getId(), false);
                // Adicionar à lista de livros alugados
    ...
    public void devolverLivro(Livro livro, LivroDAO livroDAO) {
        if (livrosAlugados.contains(livro)) {
            try {
                ...
                System.err.println("Erro ao registrar devolução no banco de dados: " + e.getMessage());
            }
        } else {
